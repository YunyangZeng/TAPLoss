...
class BaseTrainer:
    ...
    def _preload_model(self, model_path):
        ...
        assert model_path.exists(), f"The file {model_path.as_posix()} does not exist. please check path."

        model_checkpoint = torch.load(model_path.as_posix(), map_location="cpu")
        """
            The original code below is commented:
            
            Reason:
                if set strict=False, no parameters are loaded because of name mismatch,
                causing the model to stay as the initial.
                
                error:  Missing key(s) in state_dict: "module.fb_model.sequence_model.weight_ih_l0",
                     Unexpected key(s) in state_dict: "fb_model.sequence_model.weight_ih_l0"
         """
        #self.model.load_state_dict(model_checkpoint["model"], strict=False)
        
        "Start of modification (by Yunyang Zeng)"
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            self.model.module.load_state_dict(model_checkpoint["model"])
        else:
    ...
    def metrics_visualization(self, noisy_list, clean_list, enhanced_list, metrics_list, epoch, num_workers=10,
        ...
        pysepm_stoi_mean = 0.0
        pysepm_pesq_mean = 0.0
