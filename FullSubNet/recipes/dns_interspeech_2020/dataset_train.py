...
from audio_zen.acoustics.feature import norm_amplitude, tailor_dB_FS, is_clipped, load_wav, subsample, aligned_subsample
...
class Dataset(BaseDataset):
    def __init__(self,
        ...
        self.use_prepared_dataset = use_prepared_dataset
    ...
    def __getitem__(self, item):
        if self.use_prepared_dataset:
            clean_file = self.clean_dataset_list[item]
            clean_y = load_wav(clean_file, sr=self.sr)
            if self.reverb_proportion > 0:
                raise NotImplementedError

            noisy_file = self.noisy_dataset_list[item]

            clean_fileid = int(clean_file.split("fileid_")[-1].split(".wav")[0])
            noisy_fileid = int(noisy_file.split("fileid_")[-1].split(".wav")[0])
